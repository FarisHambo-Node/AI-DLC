...
## NEW: Harness Runtime Layer (Inside Each Agent Container)

**IMPORTANT ARCHITECTURAL ADDITION**

Each agent container now includes a **Harness Runtime Layer**.  
This layer sits *between the agent logic and the LLM model* and is responsible for managing execution context, skills, tool routing, and memory discipline.

Updated execution structure:

User → Chat UI → Backend Orchestrator → Redis Queues → Agent Container → **HARNESS RUNTIME** → Model + Tools + Skills

The harness runtime performs:

- Skill loading (markdown-based procedural capabilities)
- Resolver-based context routing (load correct docs automatically)
- Tool registry access (Git, Jira, CI, test runners, search, etc.)
- Context window management
- Prompt/session lifecycle loop
- Guardrails and safety enforcement
- Structured agent memory handling

This ensures agents are **not simple prompt-to-model wrappers**, but structured execution environments.

### Updated Agent Container Structure

Agent Container now contains:
...
Queues still control lifecycle and orchestration.  
Harness controls execution quality and capability.

---
** — spins up agent containers per flow, passes them the right context, collects their outputs, and decides what happens next.
